...
class DualRingSpinner(ThemedWidget):
    ...
    def paintEvent(self, event):
        ...
        # Compute a base radius from widget size. We'll later shrink the actual
        # draw radius to account for pen widths so arcs don't clip.
        base_radius = size / 2 - 2

        outer_radius = base_radius
        inner_radius = base_radius * 0.65

        outer_width = max(2, base_radius * 0.12)
        inner_width = max(2, base_radius * 0.08)
        track_width = max(1, base_radius * 0.06)

        # Prevent pen strokes from being clipped by shrinking radii based on
        # the thickest stroke we'll draw.
        max_outer_pen = max(outer_width, track_width) / 2.0
        outer_radius = max(1.0, outer_radius - max_outer_pen - 1.0)
        inner_radius = max(1.0, inner_radius - inner_width / 2.0 - 1.0)
