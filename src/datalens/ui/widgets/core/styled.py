...
class StyledMixin:
    ...
    # V1-style defaults (pill buttons + segmented toggles).
    # V1 uses a 32px-high pill with a 16px radius.
    _default_pill_radius: int = 16
    _default_pill_vpadding: int = 6
    _default_pill_hpadding: int = 18
        ...
        *,
        ...
        if self._override_hover_bg:
            ...
            if hasattr(theme, "with_alpha_hex"):
                hover_alpha = getattr(getattr(theme, "opacity", object()), "hover_fill", 0.9)
                # V1-style: hovering an *inactive* surface shows a subtle tint
                # of the selected/accent colour rather than simply making the
                # base colour translucent.
                hover_base = theme.with_alpha_hex(selected_bg, hover_alpha)
                # Keep selected hover stable by default (widgets may override).
